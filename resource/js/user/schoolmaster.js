schoolmasterJs = {
	init:function(){
		...
		$(".ui-footer").remove();
		$("#schoolmaster").height(document.body.clientHeight);
		$.get(mainJs.getApiUrl('/urming_quan/user/getWxJsApi'), {url:location.href.split('#')[0],random:Math.random()}, function(data){
			...
			wx.ready(function(){
				wx.onMenuShareAppMessage({
					title: "将创新的思维，传到每个角落", // 分享标题
					desc: "4G创新创业教育人才服务平台---创新学堂诚邀您的入驻", // 分享描述
					link: "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx3b462eee381207f3&redirect_uri=http%3A%2F%2Fm.edu.euming.com%2Fschoolmaster.html&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect", // 分享链接
					imgUrl: "http://m.edu.euming.com/resource/images/schoolmaster/share.png", // 分享图标
		...
		var $f = $('#schoolmaster .content');
		var $p = $.mobile.activePage;
		var msg;

		var name = $.trim($f.find('#name').val());
		var contactsname = $.trim($f.find('#contactsname').val());
		var contactstel = $.trim($f.find('#contactstel').val());
		...
		var e = {name:name,contactsname:contactsname,contactstel:contactstel,wechatCode:params.code,systemType:"Pkuie"};
		$.post(mainJs.getApiUrl("/urming_quan/user/addInstitutionInfo"), e,function(e) {
			var r = $.parseJSON(e);
