teacherdayJs = {
	init:function(){
		...
		$(".ui-footer").remove();
		$("#teacherday").height(document.body.clientHeight);
		var shareTitle="感恩教师节";
		var shareDesc="感恩教师节,创新学堂910元金币大放送，快为你心中的Ta送上一份节日祝福吧！";
		...
		if(typeof (params.activity) != "undefined"){
			switch (params.activity){
				case '0':
					shareTitle="感恩教师节";
					shareDesc="感恩教师节,创新学堂910元金币大放送，快为你心中的Ta送上一份节日祝福吧！";
					shareImgUrl="http://m.edu.euming.com/resource/images/teacherday/bg.jpg";
					$("#teacherday").css("background-image","url(resource/images/teacherday/bg.jpg)");
					$(".teachername input").attr("placeholder","您的老师");
					break;
				case '1':
					shareTitle="创新训练营奖学金币";
					shareDesc="祝贺创业训练营圆满结束，创新学堂为您准备上了1001枚奖学金币，快为学员们送上一份祝福吧";
					shareImgUrl="http://m.edu.euming.com/resource/images/teacherday/thumbnail1.png";
					$("#teacherday").css("background-image","url(resource/images/teacherday/bg1.jpg)");
					$(".teachername input").attr("placeholder","您的祝福接收人");
		...
		var $f = $('#teacherday .vForm');
		var $p = $.mobile.activePage;
		var msg;

		var teachername = $.trim($f.find('.vTr.teachername input').val());
		var bless = $.trim($f.find('.vTr.bless textarea').val());
		...
		//var params = {'teachername':teachername,'bless':bless};
		var params = mainJs.parseUrlSearch();
		var activity = '';
		if(typeof (params.activity) != "undefined"){
			activity = '%26activity%3d'+params.activity;
		}
		window.location.href='https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx3b462eee381207f3&redirect_uri=http%3A%2F%2Fm.edu.euming.com%2FteacherdayShare.html%3Fteachername%3d'+teachername+'%26bless%3d'+bless+activity+'&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect';
		...
		viewJs.navigator.next({
			next: {
				...
				url: './teacherday.html', id: 'teacherday',
				options: {data: mainJs.parseUrlSearch()}
			}
		});
	}
