learningCardReceiveJs = {
	init:function(){
		this.toggleEvents(true);
		this.initPage();
	},initPage:function(){
		$(".ui-footer").remove();
		$("#learningCardReceive").parent().parent().css("font-size",document.getElementsByTagName('html')[0].getBoundingClientRect().width / 16 + 'px');//兼容
		...
		$.get(mainJs.getApiUrl('/urming_quan/user/getLearningCard'), {learningCardID:params.learningCardID,secretKey:params.secretKey}, function(data){
			var learningCard = $.parseJSON(data).learningCard;
			$(".content-title > span").html(learningCard.service.serviceVersion.serviceName);
			var teacherUser = learningCard.service.serviceVersion.teacherUser;
			if(teacherUser!=undefined){
				$(".teacher-avatar > img").attr("src",mainJs.getProfilePicUrl({url:teacherUser.profileImageUrl, sex:teacherUser.sex}));
				$(".teacher-info-right > span").html(teacherUser.realname);
				$(".teacher-info-right > p").html(teacherUser.description);
			}
			$(".teacher-info-bottom > span").html(learningCard.service.serviceVersion.serviceDesc);
		}).error(function(){
		...
		$.get(mainJs.getApiUrl('/urming_quan/user/getWxJsApi'), {url:location.href.split('#')[0],random:Math.random()}, function(data){
			...
			wx.ready(function(){
				wx.onMenuShareAppMessage({
					title: '《创新学堂》听课证发给您', // 分享标题
					desc: '亲爱的同学们,你是否拥有丰富的创新思维、正确的创业理念？创新学堂为你带来丰富的实践课程，愿你喜欢！', // 分享描述
					link: 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx3b462eee381207f3&redirect_uri=http%3A%2F%2Fm.edu.euming.com%2FlearningCardReceive.html%3FlearningCardID%3d'+params.learningCardID+'%26secretKey%3d'+params.secretKey+'&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect', // 分享链接
		...
		if(isBind){
			...
			setTimeout(function(){
				me.toggleEvents();
				$p.delegate('.btn-div', 'vclick', me.submit);
				$p.delegate('.footer > span:nth-child(1)', 'vclick', me.learningCardExplain);
			}, 500);
		}
	},submit:function(){
		$('.spinner').show();
		...
		var params = mainJs.parseUrlSearch();
		var next = {url:'./learningCardReceiveSucc.html', id:'learningCardReceiveSucc', options:{data:params}};
		if(navigator.userAgent.toLowerCase().match(/MicroMessenger/i)=="micromessenger" && params.code){//weixin
			var t = dmJs.sStore;
			...
			if(user == null){
				dmJs.sStore.save("regNext",'learningCardReceiveSucc');
				dmJs.sStore.save("regNextParams",mainJs.parseUrlSearch());
				dmJs.sStore.toLogin({url:'./learningCardReceiveSucc.html', options:{data:mainJs.parseUrlSearch()}});
				return;
			}else{
				viewJs.navigator.next({
					next: next
		...
		viewJs.navigator.next({
			next: {
				...
				url: './learningCardReceive.html', id: 'learningCardReceive',
				options: {data: mainJs.parseUrlSearch()}
			}
		});
	}
