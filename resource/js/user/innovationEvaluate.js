innovationEvaluateJs = {
	init:function(){
			...
			$.get(mainJs.getApiUrl('/urming_quan/user/getWxJsApi'), {url:location.href.split('#')[0],random:Math.random()}, function(data){
				...
				wx.ready(function(){
					wx.onMenuShareAppMessage({
						title: '你是明天的马云？还是未来的巴菲特？', // 分享标题
						desc: '是金子就要发光，创新学堂创新创业力测评，帮你认清创新之路！', // 分享描述
                        a :'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxe1c5897aa5d978a…e%3D2&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect',
						link: 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx3b462eee381207f3&redirect_uri=http%3A%2F%2Fm.edu.euming.com%2FinnovationEvaluate.html%3FsourceTag%3d'+params.sourceTag+'&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect', // 分享链接
						imgUrl: 'http://m.edu.euming.com/resource/images/innovationEvaluate/innovationEvaluateShare.jpg', // 分享图标
		...
		if(isBind){
			...
			setTimeout(function(){
				...
				$p.delegate('.innovationEvaluateExplain', 'vclick', me.toInnovationEvaluateExplain);
			}, 500);
		}
	},submit:function(){
		$('.spinner').show();
		...
		var params = mainJs.parseUrlSearch();
		var next = {url:'./calendar.html', id:'calendar', options:{data:params}};
		var t = dmJs.sStore;
		dmJs.sStore.save("sourceTag",params.sourceTag);
		dmJs.sStore.save("acTag",'');
		if(navigator.userAgent.toLowerCase().match(/MicroMessenger/i)=="micromessenger" && !params.code){//weixin
			var r = mainJs.getApiUrl("/urming_quan/user/joinInInnovationEvaluation");
			var e= {};
			e.systemType = "Pkuie";
			e.wechatCode = params.code;
			e.sourceTag = params.sourceTag;
			e.acTag = '';
			$.post(r, e,
				function(e) {
					var r = $.parseJSON(e);
			...
			if(user == null){
				dmJs.sStore.save("regNext",'innovationEvaluate');
				dmJs.sStore.toLogin({url:'./innovationEvaluate.html', options:{data:mainJs.parseUrlSearch()}});
				return;
			}
			$.get(mainJs.getApiUrl('/urming_quan/user/joinInInnovationEvaluation'), {'accessToken':dmJs.sStore.getUserInfo().accessToken}, function(data){
	...
	},toInnovationEvaluateExplain:function(){
		viewJs.navigator.next({
			next: {
				...
				url: './innovationEvaluate.html', id: 'innovationEvaluate',
				options: {data: mainJs.parseUrlSearch()}
			}
		});
	}
