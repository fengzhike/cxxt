christmasJs = {
	init:function(){
		...
		if(params.sourceTag=="418d5740a7771"){//渠道关闭
			window.location.href="http://m.edu.euming.com";
			return;
		}
		...
		if(isBind){
			...
			setTimeout(function(){
				...
				$p.delegate('.school-item', 'click', me.selSchool);
				$p.delegate('.christmasExplain', 'vclick', me.christmasExplain);
		...
		var handler;
		handler = setTimeout(function(){console.log("xxx");
			me._matchSchool($el);
		}, 200);
		...
		$el.data('lastVal', val);
	},selSchool:function(event){
		var $p = $.mobile.activePage;
		$p.find('input[name=school]').val($.trim($(this).text()));
		var $list = $('#match_school_list');
		$list.empty().hide();
		event.stopPropagation();
	},christmasExplain:function() {
		viewJs.navigator.next({
			next: {
				url: "./christmasExplain.html",
				id: "christmasExplain",
				options: {data: mainJs.parseUrlSearch()}
			}, last: {
				url: './christmas.html', id: 'christmas',
				options: {data: mainJs.parseUrlSearch()}
			}
		});
	},toIndex:function(){
