operateByTeacherJs = {
	init:function(){
		...
		$.get(mainJs.getApiUrl('/urming_quan/user/getWxJsApi'), {url:location.href.split('#')[0],random:Math.random()}, function(data){
			...
			wx.ready(function(){
				wx.onMenuShareAppMessage({
					title: '创新导师招募开始啦！！！', // 分享标题
					desc: '这里有众多的创新人才等待您；这里有丰厚的课酬回报等待您；赶快报名吧！', // 分享描述
					link: 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx3b462eee381207f3&redirect_uri=http%3A%2F%2Fm.edu.euming.com%2FoperateByTeacher.html%3FsourceTag%3d'+params.sourceTag+'&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect', // 分享链接
					imgUrl: 'http://m.edu.euming.com/resource/images/operate/operateByTeacherShare.jpg', // 分享图标
		...
		if(navigator.userAgent.toLowerCase().match(/MicroMessenger/i)=="micromessenger"){//weixin
			...
			e.acTag = 'teacher';
			var next = {url:'./sysMessage.html', id:'sysMessage', options:{data:params}};
			$.post(r, e,
				function(e) {
					var r = $.parseJSON(e);
					...
					if(typeof r.tempToken != "undefined"){
						...
						dmJs.sStore.save("acTag",'teacher');
