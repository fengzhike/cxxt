	...
	init:function(){
		...
		if(isBind){
			...
			setTimeout(function(){
				...
				$p.delegate('.vForm-3 > img', 'vclick', me.createSecurityCode);
		...
		dmJs._ajax({
			...
			callback:function(data){
				...
				verifyEnterpriseJs.createSecurityCode();
			}
		})
		...
		var name = f.name;
		var institutionProperty = f.institutionProperty;
		var contactRealname = f.contactRealname;
		...
		var contactEmail = f.contactEmail;
		var contactAddress = f.contactAddress;
		var content = $p.find(".content").val();

		var msg;
		if(!msg){
			msg = viewJs.validate({must:true, val:name, name:"公众号名称"});
		}
		if(!msg){
			msg = viewJs.validate({must:true, val:institutionProperty, name:"公众号性质"});
		}
		if(!msg){
			msg = viewJs.validate({must:true, val:contactRealname, name:"公众号联系人姓名"});
		}
		if(!msg){
			msg = viewJs.validate({must:true, val:verifyCode, name:"验证码"});
		}
		if(!msg){
			msg = viewJs.validate({must:true, val:contactEmail, name:"公众号联系邮箱"});
		}
		if(msg){
		...
		formData.append("institutionProperty", institutionProperty);
		formData.append("contactRealname", contactRealname);
		formData.append("verifyCode", verifyCode);
		formData.append("tempAccessToken", tempAccessToken);
		formData.append("contactEmail", contactEmail);
		formData.append("contactAddress", contactAddress);
		formData.append("content", content);
		viewJs.hideBusyMask('submit');
		dmJs.ajaxForm(formData, mainJs.getApiUrl('/urming_quan/user/institutionVerify'), function(){
			var userInfo =dmJs.sStore.getUserInfo();
...
//			}, '提示',{onlyBtnOk:true});
		...
		var f = files[0];
		verifyEnterpriseJs.makeThumb(f);
		...
		$p.find(".vForm-3 > img").attr("src",mainJs.getApiUrl("/urming_quan/system/getImgVerifyCode")+"?code="+tempCode+"&width=78&height=30&random="+Math.random());
	},getVerifyCode:function(){
		var $p = $.mobile.activePage;
		var mobile = $p.find('.phone').val();
		mobile = $.trim(mobile);
		if(mobile == ''){
			return;
		}
		$p.find('.phone input').val(mobile);
		$p.find('.securityCode input').val($p.find('.vForm-3 .vForm-securityCode').val());
