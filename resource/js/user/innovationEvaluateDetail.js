innovationEvaluateDetailJs = {
    init:function(){
        this.toggleEvents(true);
        this.fetchData();
    },toggleEvents:function(isBind){
        var $p = $.mobile.activePage;
        var me = innovationEvaluateDetailJs;
        $p.undelegate();
        if(isBind){
        ...
        dmJs._ajax({
            ...
            url:'/urming_quan/user/getInnovationEvaluationDetail',
            params:params,
            callback:function(data){
                me.initInnovationEvaluateDetailInfo(data);
            }
        });
        $.get(mainJs.getApiUrl('/urming_quan/user/getWxJsApi'), {url:location.href.split('#')[0],random:Math.random()}, function(data){
            ...
            wx.ready(function(){
                wx.onMenuShareAppMessage({
                    title: '是金子，我们就要绽放光芒~~！', // 分享标题
                    desc: '我在创新学堂进行了创新创业力测评，获得了'+$p.find("#rank").html()+'称号，你也来试试吧。', // 分享描述
                    link: location.href.split('#')[0], // 分享链接
                    imgUrl: 'http://m.edu.euming.com/resource/images/innovationEvaluate/innovationEvaluateDetail-bg.jpg', // 分享图标
    ...
    },initInnovationEvaluateDetailInfo:function(data){
        var $p = $.mobile.activePage;
        var innovationEvaluate = data.data;
        var rank = {1:'创新预备生',2:'一级创新学员',3:'二级创新学员',4:'三级创新学员',5:'四级创新学员',6:'五级创新学员',7:'六级创新学员',8:'七级创新学员',9:'八级创新学员',10:'九级创新学员',11:'一级创新导师',12:'二级创新导师',13:'三级创新导师',14:'四级创新导师',15:'五级创新导师',16:'六级创新导师',17:'七级创新导师',18:'八级创新导师',19:'九级创新导师',20:'十级创新导师'};
        var assess = '';
        if(innovationEvaluate.rank<4){
            assess = '你资历尚浅，目前距离创业还有很长的路要走，目前应继续以学习和实践为主，相信通过你的努力，有朝一日你定会有所作为。';
        }else if(innovationEvaluate.rank<7){
            assess = '你天资聪慧，已经具备一定的创业者素质，可以从小事着手开始创业演练，夯实理论基础，同时积极参加实践项目，不日便可独挡一方天地。';
        }else if(innovationEvaluate.rank<9){
        ...
        $p.find("#realname").html(innovationEvaluate.realname);
        $p.find("#serviceCount").html(innovationEvaluate.serviceCount);
        $p.find("#wantCount").html(innovationEvaluate.wantCount);
        $p.find("#questionCount").html(innovationEvaluate.questionCount);
        $p.find("#answerCount").html(innovationEvaluate.answerCount);
        $p.find("#orderCount").html(innovationEvaluate.orderCount);
        $p.find("#soldMoney").html(innovationEvaluate.soldMoney);
        $p.find("#rank").html(rank[innovationEvaluate.rank]);
