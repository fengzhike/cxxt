...
http://localhost:8080/leon/html5-make-thumb/
*/


(function(window, $, undefined) {
    var bChrome = navigator.userAgent.search(/chrome\/[\d\.]+/i);
    // 'use strict';
    // caches
    ...
    var setting = {
        ...
        size: 'auto', // CSS3 background-size: contain | cover | auto
        mark: {}, // watermark
        // text watermark.
        ...
        stretch: true, // stretch image(small versions) to fill thumbnail (size = auto | contain)
	...
	// 缩放
	var calcNewSize = function(opts, size){
	    var newSize = {width:size[0], height:size[1]};
		var rate;
		rate = Math.max(size[0]/opts.maxWidth, size[1]/opts.maxHeight);
		if(rate > 1){
			newSize.width = Math.floor(newSize.width/rate);
			newSize.height = Math.floor(newSize.height/rate);
		}
		$.extend(opts, newSize);
	};
    $.fn.makeThumb = function(file, options) {
        ...
        if (!$.support.filereader) {opts.error.apply($self, [file, null]);return;}
        var size = opts.size;
		 var dataURL = '';
        // $self.change(function() {
            ...
            var callback = function(fEvt) {
                ...
                if ($.isFunction(opts.success)) {
                    ...
                    opts.success.apply($self, [dataURL, targetSize, file, imageSize, fEvt, opts.keepCanvas ? $canvas : null]);
                }
				if(!opts.keepCanvas){
					$canvas.remove(); // delete canvas
				}
            };
			// alert('fileType:'+fileType+':'+(IMG_FILE.test(fileType)));
            if (IMG_FILE.test(fileType)) {
				var mpImg = new MegaPixImage(file);
				var drawImage = function(fEvt, exif,/*原图大小*/ originalSize,img) {
					var orientation = exif ? exif.Orientation : null ;
					calcNewSize(opts, originalSize);
                    ...
                    if(!bChrome){
                        ...
                        canvas.width =  img.width = opts.width;
                        canvas.height = img.height = opts.height;
				...
				var fronerror = function(fEvt) { // error callback
					$canvas.remove();
                    if ($.isFunction(opts.error)) opts.error.apply(null, [file, fEvt]);
                };
				var fronload = function(fEvt) { // onload success
                    // console.log(fEvt);
                    var target = fEvt.target;
                    var result = target.result;
					var startStr = result.substr(0, 11);
					if(startStr == 'data:base64'){
						// 华为荣耀四核原生浏览器fixed
						result = 'data:'+fileType +';base64'+ result.substr(11);
					}
                    // load img
                    image = new Image();
                    var exif;
                    image.onload = function() { // imgW / height
                        drawImage.apply(null, [fEvt, exif,[this.width, this.height],this]);
                    };
                    if(!bChrome){
                        try {
				...
				opts.error.apply(null, [file, null])
			}
