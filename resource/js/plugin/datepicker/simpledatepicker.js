var DatePicker = function () {
	...
	if($('datePickerStyle') == null){
		...
		a.id="datePickerStyle";
		a.innerHTML = ([
		'@media screen and (max-width: 600px) {.date-picker-wp {max-width: 80%;}}',
		'@media screen and (min-width: 600px) {.date-picker-wp {max-width: 500px;}}',
		'td.hand:hover {background-color: #DCDCDC;}td.hand:not(.noborder){border-radius:5px;}',
		'.date-picker-wp {',
		'display: none;',
		'position: absolute;',
		'background-color: rgb(250, 250, 250);',
		'left: 40px;',
		'top: 40px;margin-left: 10px;',
		'}',
		...
		'.hand {cursor: pointer; color: #17181A;}',
			// '</style>'
		].join(''));
	...
	var init = function (n, config) {
		...
		init.prototype = {
			update : function (y, m) {
				...
				_html = '<table cellpadding=0 cellspacing=0>';
				...
				_html += '<tr style="background-color: rgb(190, 141, 141);">'+fn("-1, null", "<<")+fn("null, -1", "<")+'<td title="'+this.n+'DatePicker" colspan=3 class="strong">'+year+'/'+month+'/'+date+'</td>'+fn("null, 1", ">")+fn("1, null", ">>")+'</tr>';
			...
			show : function () {
				...
				// s['left'] = is['left'] = getPos(this.el).x - 10 + 'px';
	s['width'] ="100%";
			...
			createBox : function (html) {
				jQuery('#'+this.n+"-mask").remove();
				jQuery('#'+this.n+"-box").remove();
				...
				mask.id  = this.n+"-mask";
				mask.frameBorder = 0;
				box.style.cssText = 'position:absolute;display:none;z-index:9999;background-color: rgb(255, 250, 240);';
