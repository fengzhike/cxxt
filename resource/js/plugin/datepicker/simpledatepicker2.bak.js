simpledatepickerJs = {
	init:function(){
		...
		var nowDt,$el,now,sd,ed,y,i,y120,ct,c, of,min,max,bind,getY1,getY2,getM,getD,ymd,$selVal,$y1,m,mText,disp,toHide;
		...
		ymd = $.extend({}, now);
		ct = $('<div class="smpDatePicker-overlay" style="display:none;"><div class="smpDatePicker"><div class="h"><div class="preBtn" direction="left"></div><div class="selVal">2008-2009</div><div class="nextBtn" direction="right"></div></div></div></div>')
		.appendTo(document.body);
		var c = $('<div class="c">').appendTo(ct.find('.smpDatePicker').offset({top:of.top}));
		var showStack = [];
		...
		disp = function(){
			$selVal.text([sd, ed].join('-'));ct.find('.y2,.m,.d').remove();ct.show();
		};
		bind = function(isBind){
			...
			ct.delegate('div[direction]', 'vclick', function(){
